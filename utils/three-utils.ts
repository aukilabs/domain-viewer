...
/**
 * Creates a transformation matrix from position and orientation data.
 * Works with both Portal objects and generic pose objects (domainDeviceData).
 * 
 * @param pose - Object containing position (px,py,pz) and orientation (rx,ry,rz,rw)
 * @param matrix - Output matrix to store the transformation
 * @returns boolean indicating if the transformation was successful
 */
export function matrixFromPose(pose: any, matrix: THREE.Matrix4): boolean {
